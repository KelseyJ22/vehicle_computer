...
uint16_t dev_adc_getChannel(uint8_t channel) {
...
uint16_t dev_adc_filterVoltage(uint16_t oldVal, uint16_t newSample) {
    // Filter coefficients
...
uint16_t dev_adc_filterVoltageFast(uint16_t oldVal, uint16_t newSample) {
...
void dev_adc_poll(volatile ADC_Value *adc) {
    ...
    adc->brake[1] = dev_adc_getChannel(0);
    adc->throttle[1] = dev_adc_getChannel(1);

    adc->brake[1] = dev_adc_filterVoltageFast(adc->brake[1], dev_adc_getChannel(0));
    adc->brake[0] = adc->brake[1];

    adc->throttle[1] = dev_adc_filterVoltageFast(adc->throttle[1], dev_adc_getChannel(1));
